...
export default function ScenarioGenerator() {
  ...
  const generateScenario = async () => {
    ...
    try {
      ...
      const res = await fetch("/ScenarioGen/api/generateScenario", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          prompt: input.trim() // Send just the concept, not the full explanation request
        }),
      });
      ...
      // Directly use the response from the scenario
      const formattedScenario = formatScenarioOutput(data.scenario.response);
      setResponse(formattedScenario);
      setInput("");
      if (data.scenario) {
        if (data.scenario) {
          setSavedScenarios(prev => [data.scenario!, ...prev]);
        }
      }
    } catch (err) {
