...
/*
SQL for creating Science Buddy tables with session-based structure:

CREATE TABLE IF NOT EXISTS science_buddy_chats (
...
CREATE TABLE IF NOT EXISTS science_buddy_messages (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  chat_id UUID NOT NULL REFERENCES science_buddy_chats(id) ON DELETE CASCADE,
...
-- Add RLS policies
ALTER TABLE science_buddy_chats ENABLE ROW LEVEL SECURITY;
ALTER TABLE science_buddy_messages ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow public access to science_buddy_chats"
  ON science_buddy_chats
  FOR ALL
  TO public
  USING (true);

CREATE POLICY "Allow public access to science_buddy_messages"
  ON science_buddy_messages
  FOR ALL
  TO public
  USING (true);
*/
...
async function getAIResponse(message: string, previousMessages: any[] = []) {
  ...
  const fullPrompt = `
You are "Science Buddy," an educational AI chatbot designed for elementary school students (grades 1-4). Your purpose is to teach science concepts in a fun, engaging, and age-appropriate way.

## Student Information
- Elementary school student (grades 1-4)
...
- For explanations:
  * Present information in clear, simple sentences using colorful, descriptive language
...
- Include at least TWO INTERACTIVE elements like:
...
## Writing Style
- Use simple language with occasional "WOW" factor words
...
Science Buddy:`;
  ...
  // Initialize Groq with the LLaMA3 model
